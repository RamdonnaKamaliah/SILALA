...
{
    // 1. Izinkan nilai baru dulu
    DB::statement("
        ALTER TABLE users 
        MODIFY membership_type ENUM('karyawan','magang','pengunjung','anggota')
    ");

    // 2. Update data lama → baru
    DB::table('users')
    ...
    DB::statement("
        ALTER TABLE users 
        MODIFY membership_type ENUM('pengunjung','anggota') 
        DEFAULT 'pengunjung'
    ");
}
