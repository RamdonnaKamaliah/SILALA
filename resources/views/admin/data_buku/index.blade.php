            ...
            <tbody>
                @foreach ($data_buku as $buku)
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-2 border-b border-gray-300">
                            @if ($buku->foto_buku)
                                <div class="w-10 h-12 overflow-hidden rounded-lg border border-gray-200">
                                    <img src="{{ asset($buku->foto_buku) }}" alt="Foto Buku" class="w-full h-full object-cover">
                                </div>
                            @else
                        ...
                        <td>
                            <form action="{{ route('admin.data_buku.destroy', $buku->id) }}" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
