                ...
                <div class="lg:col-span-8 space-y-6">
                    ...
                    <div class="bg-white rounded-2xl shadow-xl border border-[#DDE6C5] overflow-hidden">
                        ...
                        <div class="p-6">
                            <form id="formProfile" action="{{ route('admin.profile.update') }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="mb-5 flex justify-center">
                                    <div class="relative">
                                        ...
                                        <span id="selectedFileName" class="hidden text-xs text-green-600 mt-1 block text-center"></span>
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    ...
                                    <div class="md:col-span-2">
                                        ...
                                        <label class="inline-flex items-center gap-2 cursor-pointer bg-gradient-to-r from-[#A4B465] to-[#8C9E55] text-white px-4 py-2 rounded-lg shadow-md hover:shadow-lg hover:scale-105 transition-all duration-300 text-sm font-medium w-fit">
                                            <i class="fas fa-camera"></i> 
                                            <span id="btnFotoText">Pilih Foto Baru</span>
                                            <input type="file" id="fotoInput" name="foto" class="hidden" accept="image/jpeg,image/jpg,image/png,image/gif">
                                        </label>
                            ...
                            {{-- DEBUG INFO --}}
                            <div id="debugInfo" class="hidden mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded text-xs">
...
<script>
    ...
    document.getElementById('fotoInput').addEventListener('change', function(e) {
        ...
        if (file) {
            ...
            reader.onload = function(e) {
                ...
                document.getElementById('previewImageForm').src = e.target.result;
            }
            reader.readAsDataURL(file);
            
            // Tampilkan nama file
            document.getElementById('btnFotoText').textContent = file.name;
            document.getElementById('selectedFileName').textContent = 'File dipilih: ' + file.name;
            document.getElementById('selectedFileName').classList.remove('hidden');
            
            // Enable button save
            const btnSave = document.getElementById('btnSaveProfile');
            btnSave.disabled = false;
            btnSave.classList.remove('disabled:bg-gray-400');
    ...
    profileInputs.forEach(inputId => {
        document.getElementById(inputId)?.addEventListener('input', function() {
            ...
            document.getElementById('btnSaveProfile').classList.remove('disabled:bg-gray-400');
            document.getElementById('btnSaveProfile').classList.add('bg-gradient-to-r', 'from-[#A4B465]', 'to-[#8C9E55]', 'hover:from-[#8C9E55]', 'hover:to-[#6E7C45]');
        });
    });

    // Enable button password saat input berubah
    const passwordInputs = ['currentPassword', 'newPassword', 'confirmPassword'];
    passwordInputs.forEach(inputId => {
        document.getElementById(inputId)?.addEventListener('input', function() {
            const allFilled = passwordInputs.every(id => document.getElementById(id).value.length > 0);
