    ...
    <div class="user-dashboard p-4 md:p-6 bg-gray-50 min-h-screen">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            ...
            <div x-data="{ open: false }" class="flex flex-wrap items-center gap-3 w-full md:w-auto">
                ...
                <!-- Modal Import -->
                <div x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
                    ...
                    <table class="w-full text-sm text-left text-gray-700" id="dataTable">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-[#F5F7ED] transition">
                                ...
                                <td class="px-4 py-3">
                                    <div class="w-16 h-20 overflow-hidden rounded-lg border mx-auto">
                                        @if ($buku->foto_buku && file_exists(public_path($buku->foto_buku)))
                                            <img src="{{ asset($buku->foto_buku) }}" class="w-full h-full object-cover"
                                                 onerror="this.onerror=null; this.src='{{ asset('images/default-book.jpg') }}';">
                                        @else
                                            <img src="{{ asset('assets/image_default/image_default_book.jpeg') }}" class="w-full h-full object-cover">
                                        @endif
                                    </div>
                                </td>

                                <td class="px-4 py-3 font-medium text-slate-800">
                                    <div class="max-w-xs truncate" title="{{ $buku->judul_buku }}">{{ $buku->judul_buku }}</div>
                                </td>

                                <td class="px-4 py-3 text-gray-600">
                                    <div class="max-w-xs truncate" title="{{ $buku->penulis }}">{{ $buku->penulis }}</div>
                                </td>

                                <td class="px-4 py-3 text-gray-600 hidden md:table-cell">
                                    <div class="max-w-xs truncate" title="{{ $buku->penerbit }}">{{ $buku->penerbit }}</div>
                                </td>

                                <td class="px-4 py-3 text-gray-600 hidden lg:table-cell">{{ $buku->tahun_terbit }}</td>

                                <td class="px-4 py-3 text-gray-600 hidden lg:table-cell">
                                    @if ($buku->kategoris && $buku->kategoris->count())
                                        <div class="max-w-xs truncate" title="{{ $buku->kategoris->pluck('nama_kategori')->join(', ') }}">
                                ...
                                <td class="px-4 py-3 text-gray-600 hidden xl:table-cell">{{ $buku->edisi ?? '-' }}</td>
                                ...
                                <td class="px-4 py-3 text-center">
                                    <div class="flex justify-center gap-2">
                                        ...
                                        <form action="{{ route('admin.data_buku.archive', $buku->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('PUT')
                                            <button type="submit"
