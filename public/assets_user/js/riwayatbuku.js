...
document.getElementById('modalOverlay').addEventListener('click', tutupModal);
...
window.pilihKamera = async function(facingMode) {
    ...
    if (!bukuDipilih) {
        Swal.fire({ icon: "warning", title: "Belum Memilih Buku", text: "Silakan pilih buku terlebih dahulu." });
        return;
    }
    ...
    document.getElementById('btnKameraDepan').classList.remove('bg-[#4C6444]', 'text-white');
    document.getElementById('btnKameraBelakang').classList.remove('bg-[#4C6444]', 'text-white');
    if (facingMode === 'user') document.getElementById('btnKameraDepan').classList.add('bg-[#4C6444]', 'text-white');
    else document.getElementById('btnKameraBelakang').classList.add('bg-[#4C6444]', 'text-white');
    ...
    try {
        streamAktif = await navigator.mediaDevices.getUserMedia({ video: { facingMode: facingMode } });
        const videoElement = document.getElementById('kameraStream');
        videoElement.srcObject = streamAktif;
        videoElement.classList.remove('hidden');
    } catch (err) {
        Swal.fire({ icon: "error", title: "Tidak Bisa Mengakses Kamera", text: "Pastikan izin kamera sudah diberikan." });
        document.getElementById('kameraArea').classList.add('hidden');
    }
};
...
window.ambilFoto = function() {
    if (!streamAktif) { Swal.fire({ icon: "warning", title: "Kamera Belum Aktif", text: "Silakan pilih kamera terlebih dahulu." }); return; }
...
window.kirimFoto = async function() {
    if (!fotoDiambil || !bukuDipilih) {
        Swal.fire({ icon: "warning", title: "Belum Lengkap", text: "Ambil foto dan pilih buku terlebih dahulu." }); return;
    }
    Swal.fire({
        title: "Kirim Foto?",
        ...
        Swal.fire({ title: "Mengirim...", text: "Foto sedang dikirim ke server.", allowOutsideClick: false, didOpen: () => Swal.showLoading() });
        try {
            const canvas = document.getElementById('fotoCanvas');
            ...
            const response = await fetch("/kembalikan-buku-foto", { method: "POST", body: formData });
            const data = await response.json();
            if (data.success) Swal.fire({ icon: "success", title: "Berhasil!", text: "Foto berhasil dikirim. Menunggu konfirmasi admin.", confirmButtonColor: "#4C6444" }).then(() => location.reload());
            else Swal.fire({ icon: "error", title: "Gagal Mengirim", text: data.message || "Terjadi kesalahan." });
