...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
  ...
  <article class="recommend-card bg-white dark:bg-[#15202B] rounded-2xl p-4 md:p-6 flex flex-col sm:flex-row items-center sm:items-start gap-4 md:gap-5 w-full h-full
    ...
    <div class="cover w-24 h-36 md:w-32 md:h-44 flex-shrink-0 transform transition-transform duration-500 hover:scale-105">
      @if ($buku->foto_buku)
        <img src="{{ asset($buku->foto_buku) }}" alt="{{ $buku->judul_buku }} - cover" class="w-full h-full object-cover rounded-lg shadow-md">
      @else
        <img src="{{ asset('assets/default-cover.jpg') }}" alt="Default Cover" class="w-full h-full object-cover rounded-lg shadow-md">
      @endif
    </div>
    ...
    <div class="meta w-full text-center sm:text-left flex flex-col justify-between">
      <div>
        <h3 class="text-lg md:text-xl font-semibold text-gray-900 dark:text-white transition-colors duration-300">{{ $buku->judul_buku }}</h3>
        <p class="text-sm md:text-base text-gray-700 dark:text-gray-300 mt-1">By {{ $buku->penulis }}</p>
      </div>
