...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  @include('layout_dashboard.partial_dashboard.link')
  <title>SILALA - Ubah Informasi Anda</title>
  @vite(['resources/css/app.css', 'resources/js/app.js'])
  <link rel="stylesheet" href="{{ asset('assets_user/css/dashboard.css') }}">
</head>

<body class="min-h-screen overflow-x-hidden font-[Ubuntu,sans-serif] bg-[#FFF8E8]">
  @include('layout_dashboard.partial_dashboard.header')

  <main
    class="pt-10 pb-10 px-8 bg-cream
           relative top-[90px] mb-24
           md:ml-[320px] md:mr-3 md:rounded-3xl
           flex justify-center transition-all duration-300 z-30">
